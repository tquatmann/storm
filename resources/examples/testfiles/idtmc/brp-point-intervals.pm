...
// gxn/dxp 23/05/2001
// DTMC with interval probabilities for channels (Storm IMDP extension)
...
module channelK
	...
	// (success prob in [0.98, 0.98], loss prob in [0.02, 0.02])
	[aF] (k=0) -> [0.98, 0.98] : (k'=1) + [0.02, 0.02] : (k'=2);
...
module channelL
	...
	// (success prob in [0.99, 0.99], loss prob in [0.01, 0.01])
	[aA] (l=0) -> [0.99, 0.99] : (l'=1) + [0.01, 0.01] : (l'=2);
