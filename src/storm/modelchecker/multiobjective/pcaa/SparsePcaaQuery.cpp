...
std::unique_ptr<CheckResult> SparsePcaaQuery<SparseModelType, GeometryValueType>::check(Environment const& env, bool produceScheduler) {
    ...
    while (!abortIterations(refinementSteps.size())) {
        ...
        // Solve WSO instance
        this->weightVectorChecker->setWeightedPrecision(storm::utility::convertNumber<typename SparseModelType::ValueType>(epsilonWso));
        weightVectorChecker->check(env, storm::utility::vector::convertNumericVector<typename SparseModelType::ValueType>(weightVector));
        ...
        if (GeometryValueType diff = optimalWeightedSum - storm::utility::convertNumber<GeometryValueType>(weightVectorChecker->getOptimalWeightedSum());
            diff > epsilonWso / 10) {
    ...
    std::vector<std::vector<typename SparseModelType::ValueType>> achievablePoints;
    achievablePoints.reserve(refinementSteps.size());
    for (auto const& step : refinementSteps) {
        achievablePoints.push_back(storm::utility::vector::convertNumericVector<typename SparseModelType::ValueType>(
            transformObjectiveValuesToOriginal(objectives, step.achievablePoint)));
    }
    return std::unique_ptr<CheckResult>(new ExplicitParetoCurveCheckResult<typename SparseModelType::ValueType>(
        this->originalModel.getInitialStates().getNextSetIndex(0), std::move(achievablePoints)));
}

template<typename GeometryValueType>
GeometryValueType getWsoPrecisionFactor(Environment const& env, uint64_t numObjectives) {
    GeometryValueType gamma = storm::utility::convertNumber<GeometryValueType>(0.5);  // Heuristic parameter for approximation tradeoff.
    ...
    return gamma / storm::utility::convertNumber<GeometryValueType>(std::sqrt(numObjectives));
}
...
typename SparsePcaaQuery<SparseModelType, GeometryValueType>::AnswerOrWeights SparsePcaaQuery<SparseModelType, GeometryValueType>::tryAnswerOrNextWeights(
    ...
    if (refinementSteps.size() < objectives.size()) {
        ...
        GeometryValueType const eps_wso = storm::utility::convertNumber<GeometryValueType>(env.modelchecker().multi().getPrecision()) *
                                          getWsoPrecisionFactor<GeometryValueType>(env, objectives.size());
...
auto findSeparatingHalfspace(auto const& refinementSteps, std::vector<GeometryValueType> const& point) {
    ...
    weightVariables.reserve(dim);
    storm::expressions::Expression sumOfVariables;
    for (uint64_t i = 0; i < dim; ++i) {
        weightVariables.push_back(solver.addBoundedContinuousVariable("w" + std::to_string(i), zero, one));
        if (i == 0) {
            sumOfVariables = weightVariables.back();
        } else {
            sumOfVariables = sumOfVariables + weightVariables.back();
        }
    }
    solver.addConstraint("", sumOfVariables <= solver.getManager().rational(one));
    auto distVar = solver.addUnboundedContinuousVariable("d", one);
    for (auto const& step : refinementSteps) {
        storm::expressions::Expression sum;
        for (uint64_t i = 0; i < dim; ++i) {
            auto expr_i = solver.getManager().rational(point[i] - step.achievablePoint[i]) * weightVariables[i];
            if (i == 0) {
                sum = expr_i;
            } else {
                sum = sum + expr_i;
        ...
        solver.addConstraint("", distVar <= sum);
    }
    solver.update();
    solver.optimize();
    ...
    if (solver.isOptimal()) {
        ...
        for (auto const& w_i : weightVariables) {
            normalVector.push_back(solver.getContinuousValue(w_i));
        }
...
SparsePcaaQuery<SparseModelType, GeometryValueType>::tryAnswerOrNextWeightsAchievability(Environment const& env, std::optional<uint64_t> const optObjIndex,
    ...
    if (referencePointInUnderApproximation) {
        ...
        if (optObjIndex.has_value()) {
            ...
            auto resultForOriginalModel =
                storm::utility::convertNumber<typename SparseModelType::ValueType>(transformObjectiveValueToOriginal(objectives[optObjIndex.value()], result));
            ...
            return std::unique_ptr<CheckResult>(new ExplicitQualitativeCheckResult(this->originalModel.getInitialStates().getNextSetIndex(0), true));
        }
    }

    // We found a separating halfspace that we can use to refine the approximation
    GeometryValueType const gamma = getWsoPrecisionFactor<GeometryValueType>(env, objectives.size());
    GeometryValueType eps_wso = storm::utility::convertNumber<GeometryValueType>(env.modelchecker().multi().getPrecision()) * gamma;
    ...
    if (separatingHalfspace->distance(referencePoint) < eps_wso) {
        ...
        GeometryValueType const distanceToOverApprox = separatingHalfspace->distance(optResPair.first);
        eps_wso = std::min<GeometryValueType>(eps_wso, distanceToOverApprox * gamma);
    }
    return WeightedSumOptimizationInput{
...
typename SparsePcaaQuery<SparseModelType, GeometryValueType>::AnswerOrWeights SparsePcaaQuery<SparseModelType, GeometryValueType>::tryAnswerOrNextWeightsPareto(
    ...
    for (auto const& vertex : vertices) {
        ...
        if (produceScheduler) {
            ...
            STORM_LOG_THROW(stepIt != refinementSteps.end(), storm::exceptions::UnexpectedException,
    ...
    return std::unique_ptr<CheckResult>(new ExplicitParetoCurveCheckResult<typename SparseModelType::ValueType>(
        ...
        transformObjectivePolytopeToOriginal(this->objectives, underApproximation)->template convertNumberRepresentation<typename SparseModelType::ValueType>(),
        transformObjectivePolytopeToOriginal(this->objectives, overApproximation)
