...
const std::string ModelCheckerSettings::scrambleSignOptionName = "scramblesign";
...
ModelCheckerSettings::ModelCheckerSettings() : ModuleSettings(moduleName) {
    ...
    this->addOption(
        storm::settings::OptionBuilder(moduleName, scrambleSignOptionName, false, "If set, every other reward gets a flipped sign").setIsAdvanced().build());
}
...
bool ModelCheckerSettings::isScrambleSignOptionSet() const {
    return this->getOption(scrambleSignOptionName).getHasOptionBeenSet();
}
