   ...
   public:
    ...
    VectorType<T> toVector() {
        ...
        while (true) {
            ...
            checkResult(_archive, bytesRead);
            if (bytesRead == 0) {
                STORM_LOG_THROW(offsetBytes == 0, storm::exceptions::FileIoException,
                                "Archive entry could not be extracted as vector of a " << sizeof(DataType) << "-bytes type: " << offsetBytes
