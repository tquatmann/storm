...
class DARewardProduct {
   public:
    DARewardProduct(storage::SparseMatrix<ValueType> transitionMatrix, std::vector<uint64_t> const& stateToModelState,
                    std::vector<uint64_t> const& actionToModelAction, std::vector<std::list<uint64_t>> const& reachingAccEcChoices,
