...
class StateActionPair {
   ...
   public:
    ...
    uint_fast64_t getState() const { return stateActionPair.first; }

    uint_fast64_t getAction() const { return stateActionPair.second; }

    friend bool operator==(StateActionPair const& p1, StateActionPair const& p2) { return p1.stateActionPair == p2.stateActionPair; }
...
struct hash<storm::storage::StateActionPair> {
    size_t operator()(storm::storage::StateActionPair const& sap) const { return (sap.getState() << 3 ^ sap.getAction()); }
};
