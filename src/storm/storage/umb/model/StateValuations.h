...
class StateValuations {
   public:
    class Valuation {
       ...
       private:
        Valuation(std::span<char const> bytes, StateValuationsDescription const& description) : bytes(bytes), description(description) {}

        using BitIndex = uint64_t;
        using DescriptionIterator = typename decltype(StateValuationsDescription::variables)::const_iterator;
        ...
        void advancePadding(BitIndex& currentValueStart, DescriptionIterator& descriptionIterator) const {
            if (descriptionIterator != description.variables.cend() && std::holds_alternative<StateValuationsDescription::Padding>(*descriptionIterator)) {
                currentValueStart += std::get<StateValuationsDescription::Padding>(*descriptionIterator).padding;
                ++descriptionIterator;
                advancePadding(currentValueStart, descriptionIterator);
            }
        }
        ...
        void invokeForCurrent(BitIndex& currentValueStart, DescriptionIterator const& descriptionIterator, auto const& callback) const {
            using enum StateValuationsDescription::Variable::Type;
            STORM_LOG_ASSERT(std::holds_alternative<StateValuationsDescription::Variable>(*descriptionIterator), "Unexpected type of variable description.");
            auto const& variableDescription = std::get<StateValuationsDescription::Variable>(*descriptionIterator);
            STORM_LOG_ASSERT(currentValueStart < bytes.size() * 8, "Unexpected bit index.");
            if (variableDescription.size.has_value() && variableDescription.size.value() <= 64) {
                // Fast path for fixed-size, up to 64 bit values
                uint64_t rawContent = readUint64(currentValueStart, variableDescription.size.value());
                currentValueStart += variableDescription.size.value();
                switch (variableDescription.type) {
                    ...
                    case Uint:
                        ...
                        break;
                    case Int: {
                        uint64_t const mostSignificantBitMask = 1ull << (variableDescription.size.value() - 1);
            ...
            if constexpr (onlyTrivialTypes) {
                ...
                switch (variableDescription.type) {
                    case Bool:
                        callback(readInteger<false>(currentValueStart, variableDescription.size.value()) != Integer(0));
                        return;
                    case Uint:
                        callback(Integer(readInteger<false>(currentValueStart, variableDescription.size.value()) +
                                         storm::utility::convertNumber<Integer>(variableDescription.offset.value_or(0))));
                        return;
                    case Int:
                        callback(Integer(readInteger<true>(currentValueStart, variableDescription.size.value()) +
                                         storm::utility::convertNumber<Integer>(variableDescription.offset.value_or(0))));
                        return;
                    case Rational: {
        ...
        StateValuationsDescription const& description;
    };

    StateValuations(std::vector<char> bytes, std::optional<std::vector<uint64_t>> stateValuationIndices, StateValuationsDescription description)
        : bytes(std::move(bytes)),
          stateValuationIndices(std::move(stateValuationIndices)),
          description(std::move(description)),
          expressionManager(std::make_shared<storm::expressions::ExpressionManager>()) {
        STORM_LOG_ASSERT(this->description.alignment > 0, "Invalid alignment 0 for state valuations.");
        STORM_LOG_ASSERT(this->bytes.size() % description.alignment == 0, "Corrupted state valuation data.");
        STORM_LOG_ASSERT(
            !this->stateValuationIndices || this->stateValuationIndices->back() == this->bytes.size() / this->description.alignment,
            "Corrupted state valuation index data: " << this->stateValuationIndices->back() << " vs. " << this->bytes.size() / this->description.alignment);
        for (auto& varDesc : this->description.variables) {
            if (std::holds_alternative<StateValuationsDescription::Variable>(varDesc)) {
                variables.push_back(createVariable(*expressionManager, std::get<StateValuationsDescription::Variable>(varDesc)));
            }
        }
    }

    StateValuations(std::vector<char> bytes, StateValuationsDescription description)
        : StateValuations(std::forward<std::vector<char>>(bytes), std::nullopt, std::forward<StateValuationsDescription>(description)) {}

    uint64_t getNumberOfStates() const {
        if (stateValuationIndices) {
            return stateValuationIndices->size() - 1;
        } else {
            return bytes.size() / description.alignment;
        }
    }
    ...
    Valuation getValuation(uint64_t stateIndex) const {
        if (stateValuationIndices) {
            ...
            uint64_t const start = (*stateValuationIndices)[stateIndex] * description.alignment;
            uint64_t const end = (*stateValuationIndices)[stateIndex + 1] * description.alignment;
            ...
            uint64_t const start = stateIndex * description.alignment;
            STORM_LOG_ASSERT(bytes.size() <= start + description.alignment, "Corrupted state valuation data.");
            return Valuation(std::span<char const>(bytes.data() + start, description.alignment), description);
        }
    }
   ...
   private:
    struct Variable {
        ...
        std::reference_wrapper<StateValuationsDescription::Variable const> description;
    };

    static Variable createVariable(storm::expressions::ExpressionManager& expressionManager, StateValuationsDescription::Variable const& variableDescription) {
