...
std::shared_ptr<storm::models::sparse::Model<ValueType>> performAbstractionMinimization(
    ...
    if constexpr (!storm::IsIntervalType<ValueType>) {
        STORM_LOG_THROW(false, storm::exceptions::NotSupportedException,
                        "Epsilon-stable abstraction minimization is currently only available for IDTMCs and IMDPs.");
        return nullptr;
    } else {
        ...
        if (model->isOfType(storm::models::ModelType::Dtmc)) {
            ...
            auto options = typename storm::storage::abstraction::EpsilonStableAbstractionDecomposition<
                storm::models::sparse::Mdp<ValueType>>::EpsilonStableAbstractionOptions(*model->template as<storm::models::sparse::Mdp<ValueType>>(), formulas,
                                                                                        epsilon);
