...
class MultiObjectiveSchedRestModelCheckerTest : public ::testing::Test {
   public:
    ...
    bool testFormula(storm::Environment const& env, SparseModelType const& model, storm::logic::Formula const& formula, std::vector<Point> const& expected,
...
TYPED_TEST(MultiObjectiveSchedRestModelCheckerTest, steps) {
    ...
    expected = this->parsePoints({"0.875,0", "0,0.875", "0.125,0.75", "0.25,0.625", "0.375,0.5", "0.5,0.375", "0.625,0.25", "0.75,0.125"});
    EXPECT_TRUE(this->testFormula(env, mdp, *formulas.front(), expected, errorString)) << errorString;
}
...
TYPED_TEST(MultiObjectiveSchedRestModelCheckerTest, mecs) {
    ...
    if (this->isFlowEncoding()) {
        STORM_SILENT_EXPECT_THROW(this->testFormula(env, mdp, *formulas[formulaIndex], expected, errorString), storm::exceptions::InvalidOperationException);
    } else {
        EXPECT_TRUE(this->testFormula(env, mdp, *formulas[formulaIndex], expected, errorString)) << errorString;
    }
